...
class ConfigApp(tk.Tk):
    def __init__(self):
        ...
        self.presets = {
            "Select a Preset...": {},
            "Default CH9329": {"vid": "1A86", "pid": "E129", "manufacturer": "wch.cn", "product": "CH9329", "serial": ""},
        ...
        self.default_baud_var = tk.StringVar(value="(read from device)")
    ...
    def _update_widget_state(self, connected):
        ...
        for child in self.config_frame.winfo_children():
            if isinstance(child, (ttk.Entry, ttk.Combobox)):
                child.config(state="normal" if connected else "readonly")
    ...
    def _toggle_connection(self):
        if self.serial_connection and self.serial_connection.is_open:
            ...
            self.default_baud_var.set("(read from device)")
            self.baud_display_label.config(foreground="grey")
    ...
    def _send_and_validate_command(self, cmd: bytes, data: bytes = b""):
        ...
        try:
            ...
            self._log(f"Serial Error: {e}", "ERROR")
            return False, None
    ...
    def _read_config(self):
        ...
        self.default_baud_var.set(str(int.from_bytes(config_payload[3:7], "big")))
        self.baud_display_label.config(foreground="black")
    ...
    def _write_config(self):
        ...
        try:
            ...
            messagebox.showerror("Input Error", "VID/PID must be valid hex.")
            self._log("Invalid hex value for VID/PID.", "ERROR")
            return
        success, config_payload = self._send_and_validate_command(CMD_GET_CONFIG)
    ...
    def _create_widgets(self):
        ...
        baud_combo = ttk.Combobox(conn_frame, textvariable=self.baud_var, values=baud_values, width=15)
        ...
        preset_combo = ttk.Combobox(self.config_frame, textvariable=self.preset_var, values=list(self.presets.keys()), state="readonly")
        ...
        ttk.Entry(self.config_frame, textvariable=self.vid_var).grid(row=1, column=1, sticky="ew")
        ttk.Label(self.config_frame, text="PID (Hex):").grid(row=1, column=2, sticky="w", padx=5, pady=2)
        ttk.Entry(self.config_frame, textvariable=self.pid_var).grid(row=1, column=3, sticky="ew")
        ttk.Label(self.config_frame, text="Manufacturer:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(self.config_frame, textvariable=self.manufacturer_var).grid(row=2, column=1, columnspan=3, sticky="ew")
        ttk.Label(self.config_frame, text="Product Name:").grid(row=3, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(self.config_frame, textvariable=self.product_var).grid(row=3, column=1, columnspan=3, sticky="ew")
        ttk.Label(self.config_frame, text="Serial Number:").grid(row=4, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(self.config_frame, textvariable=self.serial_var).grid(row=4, column=1, columnspan=3, sticky="ew")
        ttk.Label(self.config_frame, text="Stored Baud:").grid(row=5, column=0, sticky="w", padx=5, pady=2)
        self.baud_display_label = ttk.Label(self.config_frame, textvariable=self.default_baud_var, relief="sunken", foreground="grey")
        self.baud_display_label.grid(row=5, column=1, columnspan=3, sticky="ew", padx=5)
