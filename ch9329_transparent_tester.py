...
# A comprehensive, CLI-driven script to test a CH9329 chip in Transparent Mode.
# Now with an expanded keymap and more thorough test cases.
...
# A more complete map for comprehensive testing.
HID_MAP = {
    "a": 0x04,
    "b": 0x05,
    "c": 0x06,
...
def run_tests(ser: serial.Serial):
    test_cases = [
        ...
        (["shift", "a"], {"A"}),
        (["c"], {"c"}),
        (["ctrl", "c"], {"\x03"}),  # pynput often captures Ctrl+C as EOT char
        (["f5"], {"f5"}),
        (["shift", "f5"], {"shift", "f5"}),
        (["alt", "tab"], {"alt", "tab"}),
        (["gui"], {"cmd"}),  # On macOS, 'gui' is captured as 'cmd'
    ]
    ...
    for i, (keys_to_send, expected_capture) in enumerate(test_cases):
        ...
        with capture_lock:
            ...
            # Special case for alt+tab, which might not be captured reliably
            if "alt" in keys_to_send and "tab" in keys_to_send and not captured_keys:
                print(f"    {C_YELLOW}NOTE: Alt+Tab is often handled by the OS and may not be captured. This is expected.{C_END}")
                # We can choose to count this as a pass if we expect this behavior
                passed_count += 1
            elif expected_capture == captured_keys:
...
def main(args):
    ...
    print(f"\n{C_YELLOW}IMPORTANT: Ensure your CH9329 is in TRANSPARENT MODE (CFG1=LOW, CFG0=HIGH).{C_END}")
